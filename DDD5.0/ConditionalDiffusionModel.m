...
% DDD5.0 version: Uses feedforwardnet for reliable training
    ...
    methods
        ...
        function [TrainX, TrainF, TrainT, TrainNoise] = prepareTrainingData(obj, XNorm, FNorm)
            ...
            if obj.D >= 50 || n_samples < 100
                aug_factor = 15;
                disp(['  [CDM] Using ULTRA data augmentation (factor=' num2str(aug_factor) ')']);
            else
                aug_factor = 8;
            end
            ...
            for i = 1:n_samples
                for j = 1:aug_factor
                    if j <= aug_factor / 2
                        t = randi(floor(obj.TimeSteps * 0.5));
                    else
                        t = randi(floor(obj.TimeSteps * 0.5)) + floor(obj.TimeSteps * 0.5);
                    end
                    t = max(1, min(t, obj.TimeSteps));
                    
                    if obj.D >= 50
                        noise = 0.8 * randn(1, obj.D);
                    else
                        noise = randn(1, obj.D);
                    end
                    
                    noisy_x = obj.SqrtAlphaBar(t) * XNorm(i, :) + obj.SqrtOneMinusAlphaBar(t) * noise;
                    noisy_x = max(min(noisy_x, 3), -3);
        ...
        function net = buildNetwork(obj)
            if obj.D >= 50
                actual_structure = [512, 1024, 1024, 512, 256];
                disp(['  [CDM] High-dim problem (D=' num2str(obj.D) '), using enhanced network: [' num2str(actual_structure) ']']);
            else
                actual_structure = obj.NetworkStructure;
            end
            
            net = feedforwardnet(actual_structure);
            net.trainFcn = 'trainscg';
            net = configure(net, rand(obj.D + obj.M + 1, 100), rand(obj.D, 100));
        ...
        function net = trainNetwork(obj, TrainX, TrainF, TrainT, TrainNoise, epochs)
            Inputs = [TrainX', TrainF', TrainT'];  % (D+M+1) x N
            Targets = TrainNoise';  % D x N

            obj.Network = configure(obj.Network, Inputs, Targets);
        ...
        function X = sample(obj, TargetObjs, ReferenceX)
            ...
            if nargin >= 3 && ~isempty(ReferenceX) && size(ReferenceX, 1) == n_samples
                ...
                noise_scale = 0.3;
                XNorm = XNorm + noise_scale * randn(n_samples, obj.D);
                disp(['  [DM Sample] Using reference initialization with ', num2str(noise_scale*100), '% noise']);
            else
                XNorm = 0.5 * randn(n_samples, obj.D);
                disp('  [DM Sample] Using random initialization (no reference provided)');
            end
            
            XNorm = max(min(XNorm, 3), -3);
            
            if obj.D >= 50
                actual_steps = min(obj.DMSteps * 2, 200);
                disp(['  [DM Sample] High-dim problem detected, using ', num2str(actual_steps), ' steps']);
            else
                actual_steps = obj.DMSteps;
            end
            
            timesteps = linspace(obj.TimeSteps, 1, actual_steps);
